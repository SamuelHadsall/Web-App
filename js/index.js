				...
				$("#category_locksets option:selected").each(function() { 
					...
					$(xml).find("lock").each(function() {
						...
						if( id == lockset_id ){
							...
							var url = $(this).find('results').find('design_url').html()
							var notes = $(this).find('results').find('notes').text();								
							$('<figure id="'+ id +'"><img src="'+img+'" alt="" /><figcaption><a class="ui-link" href="'+url+'" target="_blank">'+design+'</a><p>'+notes+'</p></figcaption></figure>').appendTo('#lockset_results');//Show results in Div
						}                                   
					});
					//alert( id );
				});
				...
				$("#category_exit_devices option:selected").each(function() { 
					...
					$(xml).find("exit-device").each(function() {
						...
						if( id == exitid ){
							...
							var notes = $(this).find('results').find('notes').text();								
							$('<fingure id="'+ id +'"><img src="'+img+'" alt="" /><figcaption><a class="ui-link" href="'+url+'" target="_blank">'+design+'</a><p>'+notes+'</p></figcaption></figure>').appendTo('#exit_device_results');//Show results in Div							
						}						                                 
					});					
					//alert( id );
				});
